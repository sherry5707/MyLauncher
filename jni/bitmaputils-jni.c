...
#include <assert.h>
#include <com_android_launcher3_jni_ImageUtils.h>  

#define LOG_TAG "bitmaputils_jni"
...
JNIEXPORT void JNICALL nativeToBlur(JNIEnv* env, jobject obj, jobject bitmapIn, jobject bitmapOut, jint radius) {
	...
	for (x = 0; x < w; x++) {
		...
		stackpointer = radius;
		for (y = 0; y < h; y++) {
			output[yi].red = dv[rsum] * 1.5f;
			output[yi].green = dv[gsum] * 1.5f;
			output[yi].blue = dv[bsum] * 1.5f;
			output[yi].alpha = (int) (255);
...
static const char *className = "com/android/launcher3/jni/ImageUtils";  

static JNINativeMethod methods[] = {   
  { "nativeToBlur", "(Landroid/graphics/Bitmap;Landroid/graphics/Bitmap;I)V", (void*) nativeToBlur},
};  
