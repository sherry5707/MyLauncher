...
public class AllAppsList {
    ...
    private static final String[] VIDEO_PACKAGE = {
            "com.fun.tv.greenorange",
        "com.tencent.qqlive",
        "com.qiyi.video",
        "com.sohu.sohuvideo",
        "com.youku.phone",
    ...
    public ArrayList<AppInfo> dataVideo =
    ...
    private static final String[][] PRELOAD_VIDEO = {
            {"com.tencent.qqlive", "com.tencent.qqlive.ona.activity.WelcomeActivity"
            , "腾讯视频", "Icon/com.tencent.qqlive.ona.activity.WelcomeActivity.png"},
            {"com.sohu.sohuvideo", "com.sohu.sohuvideo.FirstNavigationActivityGroup"
                    , "搜狐视频", "Icon/com.sohu.sohuvideo.FirstNavigationActivityGroup.png"}};
    ...
    public AllAppsList(IconCache iconCache, AppFilter appFilter) {
        ...
        for (int i = 0; i < 2; i++) {
            ...
            info.packageName = PRELOAD_VIDEO[i][0];
            info.className = PRELOAD_VIDEO[i][1];
            info.fakeTitle = PRELOAD_VIDEO[i][2];
            info.fakeIcon = PRELOAD_VIDEO[i][3];
            dataVideo.add(info);
        }
    }
    ...
    public void add(AppInfo info) {
        ...
        for (String pacakgeName:VIDEO_PACKAGE) {
            if(pacakgeName.equals(info.getPackageName())){
                if (!findFakeActivity(dataVideo, info.componentName, info.user)) {
                    dataVideo.add(info);
                }
            }
        }
    ...
    public void updatePackage(Context context, String packageName, UserHandleCompat user) {
        ...
        if (matches.size() > 0) {
            if (matches.size() == 1 && !"com.tencent.mm".equals(packageName) && !"com.vapp.aide.intl".equals(packageName)
                    && !"cn.wps.moffice_eng".equals(packageName)) {
                final LauncherActivityInfoCompat info = matches.get(0);
                ...
                if (applicationInfo == null) {
                    ...
                    applicationInfo.intent.setComponent(info.getComponentName());
                    applicationInfo.updateLunchShortcut = true;
                ...
                for (int i = data.size() - 1; i >= 0; i--) {
                    ...
                    if (user.equals(applicationInfo.user)
                        ...
                        if (!findActivity(matches, component)) {
                            ...
                            data.remove(i);
                            dataVideo.remove(applicationInfo);
                        }
                    }
                }
                ...
                for (int i = 0; i < count; i++) {
                    ...
                    if (applicationInfo == null) {
                        ...
                        mIconCache.remove(applicationInfo.componentName, user);
                        mIconCache.getTitleAndIcon(applicationInfo, info, null);
                        applicationInfo.updateLunchShortcut = false;
                        modified.add(applicationInfo);
                    }
                }
            }
    ...
    private static boolean findFakeActivity(ArrayList<AppInfo> apps, ComponentName component,
                                        UserHandleCompat user) {
        final int N = apps.size();
