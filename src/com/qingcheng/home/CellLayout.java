...
import com.qingcheng.home.Workspace.State;
...
public class CellLayout extends ViewGroup {
    ...
    public State getState(){
    	return mLauncher.getWorkspace().getmState();
    }
    //piflow end sunfeng @20150812
    
    // Add for MyUI Jing.Wu 20151130 start
	...
	private View addOrDeleteEmptyScreenView;
    private int addButtonWidth;
    private int addButtonHeight;
    private void initAddButton() {
    	if (addOrDeleteEmptyScreenView == null) {
            addOrDeleteEmptyScreenView = new View(mLauncher);
            addOrDeleteEmptyScreenView.setOnClickListener(new View.OnClickListener() {
    			@Override
    			public void onClick(View v) {
    				...
    				if (mShortcutsAndWidgets.getChildCount() == 0) {
						...
						if (mWorkspace.getIdForScreen(CellLayout.this) == Workspace.EXTRA_EMPTY_SCREEN_ID) {
					        int childCount = mWorkspace.getChildCount() - mWorkspace.numCustomPages();
					        ...
					        if (countFlag) {
					        	mWorkspace.insertNewWorkspaceScreenBeforeEmptyScreen(
			    						LauncherAppState.getLauncherProvider().generateNewScreenId());
							} else {
								mWorkspace.commitExtraEmptyScreen();
							}
						} else {
							mWorkspace.deleteEmptyScreen(CellLayout.this);
							
						}
					}
    			}
    		});
            addButtonWidth = (int)mLauncher.getResources().getDimension(R.dimen.add_new_empty_screen_button_width);
            addButtonHeight = (int)mLauncher.getResources().getDimension(R.dimen.add_new_empty_screen_button_height);
		}
    }
    public void showAddButton(boolean testState) {
    	if (mLauncher.getWorkspace() == null || (testState && mLauncher.getWorkspace().getState() == State.NORMAL)) {
			return;
		}
    	if (updateAddButtonBg()) {
        	if (indexOfChild(addOrDeleteEmptyScreenView)<0) {
            	addView(addOrDeleteEmptyScreenView, addButtonWidth, addButtonHeight);
    		}
        	if (addOrDeleteEmptyScreenView.getVisibility()!=View.VISIBLE) {
        		addOrDeleteEmptyScreenView.setVisibility(View.VISIBLE);
			}
		} else {
			hideAddButton();
		}
    }
    public boolean updateAddButtonBg() {
    	if (getParent() instanceof Workspace && mShortcutsAndWidgets.getChildCount() == 0 
    			&& !mLauncher.getWorkspace().isCustomPage(this)) {
			if (mLauncher.getWorkspace().getIdForScreen(this) == mLauncher.getWorkspace().EXTRA_EMPTY_SCREEN_ID) {
				addOrDeleteEmptyScreenView.setBackgroundResource(R.drawable.add_new_page);
				return true;
			} else {
				boolean countFlagOK = mLauncher.getWorkspace().getNormalChildCount()>1;
				long firstNormalScreenID = mLauncher.getWorkspace().getScreenOrder().get(mLauncher.getWorkspace().hasCustomContent()?1:0);
				if (!countFlagOK && mLauncher.getWorkspace().getIdForScreen(this) == firstNormalScreenID) {
					return false;
				} else {
					addOrDeleteEmptyScreenView.setBackgroundResource(R.drawable.delete_empty_page);
					return true;
				}
			}
		}
    ...
    public void hideAddButton() {
    	if (addOrDeleteEmptyScreenView != null 
    			&& indexOfChild(addOrDeleteEmptyScreenView) >= 0 
    			&& addOrDeleteEmptyScreenView.getVisibility() != View.GONE) {
        	addOrDeleteEmptyScreenView.setVisibility(View.GONE);
		}
    }
    // Add for MyUI Jing.Wu 20151130 end
    ...
    public CellLayout(Context context, AttributeSet attrs, int defStyle) {
        ...
        initAddButton();
        if (!QCConfig.autoDeleteAndAddEmptyScreen) {
            mShortcutsAndWidgets.setOnHierarchyChangeListener(new ViewGroup.OnHierarchyChangeListener() {
    			@Override
    ...
    void setIsDragOverlapping(boolean isDragOverlapping) {
        if (mIsDragOverlapping != isDragOverlapping) {
            ...
            // Delete for MyUI Jing.Wu 20151228 start
            //setUseActiveGlowBackground(mIsDragOverlapping);
            // Delete for MyUI Jing.Wu 20151228 end
            invalidate();
        }
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < mFolderOuterRings.size(); i++) {
            ...
            if (child != null) {
            	child.setScaleX((float)fra.getShortcutSize()/previewOffset);
            	child.setScaleY((float)fra.getShortcutSize()/previewOffset);
    ...
    protected void dispatchDraw(Canvas canvas) {
        ...
        if (Launcher.isInEditMode()) {
            ...
            final int colGap = (measuredWidth - crossWidth) / mCountX;
            final int rowGap = (measuredHeight - crossHeight) / mCountY;
    ...
    public boolean addViewToCellLayout(View child, int index, int childId, LayoutParams params,
        ...
        if (child instanceof BubbleTextView) {
            ...
            bubbleChild.setTextVisibility(!mIsHotseat || QCConfig.supportHotseatText);
        }
        
        //sunfeng modfiy @20150802 for folder end
        if (QCLog.DEBUG) {
			QCLog.d(TAG, "addViewToCellLayout() and setScaleX("+getChildrenScale()+")");
		}
    ...
    public void removeViewsInLayout(int start, int count) {
     ...
     //M: [OP09]Edit app support @{
     /**
      * M: setTagToCellInfoForPoint.
      *
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        //Here, we also do some things for OP09 @{
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (child.equals(addOrDeleteEmptyScreenView)) {
            	child.measure(addButtonWidth, addButtonHeight);
				continue;
			}
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (child.equals(addOrDeleteEmptyScreenView)) {
            	...
            	int leftPadding = (int)(parentWidth/2)-(int)(addButtonWidth/2);
            	int topPadding = (int)(parentHeight/2)-(int)(addButtonHeight/2);
            	int rightPadding = (int)(parentWidth/2)-(int)(addButtonWidth/2);
            	int bottomPadding = (int)(parentHeight/2)-(int)(addButtonHeight/2);
            	child.layout(leftPadding, topPadding, parentWidth-rightPadding, parentHeight-bottomPadding);
            	continue;
			}
    ...
    public void setBackgroundAlpha(float alpha) {
    	if (QCLog.DEBUG) {
			QCLog.d(TAG, "setBackgroundAlpha() and alpha = "+alpha);
		}
    ...
    public boolean animateChildToPosition(final View child, int cellX, int cellY, int duration,
        ...
        if (clc.indexOfChild(child) != -1) {
            ...
            //sunfeng modify 20150818 end:
        	if (QCLog.DEBUG) {
				QCLog.d(TAG, "animateChildToPosition() and oldX:"+oldX+", newX:"+lp.x+", oldY:"+oldY+", newY:"+lp.y);
			}
            clc.setupLp(lp);
            lp.isLockedToGrid = false;
        	if (QCLog.DEBUG) {
				QCLog.d(TAG, "animateChildToPosition() and after clc.setupLp(lp);" +
						" oldX:"+oldX+", newX:"+lp.x+", oldY:"+oldY+", newY:"+lp.y);
			}
            final int newX = lp.x;
            final int newY = lp.y;
            ...
            if (oldX == newX && oldY == newY) {
            	if (QCLog.DEBUG) {
					QCLog.d(TAG, "animateChildToPosition() and oldX = newX && oldY = newY return");
				}
                lp.isLockedToGrid = true;
                return true;
            }
                ...
                switch (which) {
                    case LEFT:
                        ...
                        for (int j = cs.y; j < cs.y + cs.spanY; j++) {
                            ///M. ALSP01966379, if arrary index out of bounds, break.
                            if (j >= edge.length) {
                                LauncherLog.w(TAG, "computeEdge,arrary index out of bounds: cs ="
                                    + cs + ", which: " + which);
                                break;
                            }
                            ///M.
    ...
    ItemConfiguration findReorderSolution(int pixelX, int pixelY, int minSpanX, int minSpanY,
        ...
        LauncherLog.d(TAG, "[guo_2]findReorderSolution");
    ...
    private void copyCurrentStateToSolution(ItemConfiguration solution, boolean temp) {
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            if (temp) {
                ...
                LauncherLog.d(TAG, "[guo_2]copyCurrentStateToSolution: c = " + c);
            }
    ...
    class ReorderPreviewAnimation {
        ...
        void animate() {
            if (mShakeAnimators.containsKey(child)) {
                ...
                if (finalDeltaX == 0 && finalDeltaY == 0) {
                	if (QCLog.DEBUG) {
						QCLog.d(TAG, "ReorderPreviewAnimation animate() and finalDeltaX = 0 && finalDeltaY = 0");
					}
            ...
            va.addUpdateListener(new AnimatorUpdateListener() {
                ...
                public void onAnimationUpdate(ValueAnimator animation) {
                    ...
                    if (QCLog.DEBUG) {
						QCLog.d(TAG, "ReorderPreviewAnimation, animate, onAnimationUpdate() child.setScaleX("+s+")");
					}
                    if (!(Workspace.isFolderDragMode() || child instanceof BubbleTextView || child instanceof FolderIcon)) {
                        child.setScaleX(s);
                        child.setScaleY(s);
					}
                }
        ...
        private void completeAnimationImmediately() {
            ...
            a = s;
            if (Workspace.isFolderDragMode() || child instanceof BubbleTextView) {
                if (QCLog.DEBUG) {
    				QCLog.d(TAG, "completeAnimationImmediately() and don't set scale");
    			}
                s.playTogether(
	            ...
	            if (QCLog.DEBUG) {
					QCLog.d(TAG, "completeAnimationImmediately() and scaleX("+getChildrenScale()+")");
				}
	            s.playTogether(
    ...
    private void completeAndClearReorderPreviewAnimations() {
    	if (QCLog.DEBUG) {
			QCLog.d(TAG, "completeAndClearReorderPreviewAnimations()");
		}
