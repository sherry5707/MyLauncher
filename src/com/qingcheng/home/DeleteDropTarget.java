...
public class DeleteDropTarget extends ButtonDropTarget {
    ...
    private boolean isAllAppsApplication(DragSource source, Object info) {
    	...
    	if (info instanceof ShortcutInfo) {
			ShortcutInfo mInfo = (ShortcutInfo)info;
			Intent mIntent = mInfo.intent;
			if (mIntent!=null && mIntent.getAction()!=null 
					&& mIntent.getComponent()!=null 
					&& mIntent.getCategories()!=null) {
				isShortcut = mIntent.getAction().equals(Intent.ACTION_MAIN) && 
				!mIntent.getComponent().getClassName().isEmpty() && 
				!mIntent.getComponent().getPackageName().isEmpty() && 
				mIntent.getCategories().contains(Intent.CATEGORY_LAUNCHER);
			}
		}
        return source.supportsAppInfoDropTarget() && ((info instanceof AppInfo)||isShortcut);
    }
    ...
    private boolean isWorkspaceOrFolderApplication(DragObject d) {
    	...
    	if (d.dragInfo instanceof ShortcutInfo) {
			ShortcutInfo mInfo = (ShortcutInfo)d.dragInfo;
			Intent mIntent = mInfo.intent;
			if (mIntent!=null && mIntent.getAction()!=null 
					&& mIntent.getComponent()!=null 
					&& mIntent.getCategories()!=null) {
        ...
        return isDragSourceWorkspaceOrFolder(d) && isShortcut;
    }
    private boolean isWorkspaceOrFolderShortcutWidget(DragObject d) {
    	boolean isShortcutWidget = false;
    	if (d.dragInfo instanceof ShortcutInfo) {
			ShortcutInfo mInfo = (ShortcutInfo)d.dragInfo;
			Intent mIntent = mInfo.intent;
			if (mIntent!=null && mIntent.getAction()!=null 
					&& mIntent.getComponent()!=null 
					&& mIntent.getCategories()!=null) {
				isShortcutWidget = !mIntent.getComponent().getClassName().isEmpty() && 
				!mIntent.getComponent().getPackageName().isEmpty() && 
				!mIntent.getCategories().contains(Intent.CATEGORY_LAUNCHER);
			} else {
				isShortcutWidget = true;
			}
		}
    
        return isDragSourceWorkspaceOrFolder(d) && isShortcutWidget;
    }
        ...
        if (info instanceof ItemInfo) {
            ...
            if (item.itemType == LauncherSettings.Favorites.ITEM_TYPE_APPLICATION &&
            	...
            	if (LauncherAppState.isDisableAllApps()) {
                    ...
                    // Change for MyUI---20150708
                    //return (shortcutInfo.flags & AppInfo.DOWNLOADED_FLAG) != 0;
                    if ((shortcutInfo.flags & ApplicationInfo.FLAG_INSTALLED) == 0) {
						return addCanAcceptDrop(info);
					} else {
    ...
    private static boolean addCanAcceptDrop(Object info){
    	ShortcutInfo mShortcutInfo = (ShortcutInfo)info;
    	ApplicationInfo mApplicationInfo = null;
		PackageInfo mPackageInfo = null;
    	try {
            if (QCLog.DEBUG) {
                QCLog.d(TAG, "mShortcutInfo.intent.getComponent().getPackageName() = "+mShortcutInfo.intent.getComponent().getPackageName());
            }
            mApplicationInfo = manager.getApplicationInfo(mShortcutInfo.intent.getComponent().getPackageName(), 
            		PackageManager.GET_UNINSTALLED_PACKAGES |
                    PackageManager.GET_DISABLED_COMPONENTS |
                    PackageManager.GET_DISABLED_UNTIL_USED_COMPONENTS);
    		mPackageInfo = manager.getPackageInfo(mShortcutInfo.intent.getComponent().getPackageName(), 
    				PackageManager.GET_DISABLED_COMPONENTS |
                    PackageManager.GET_UNINSTALLED_PACKAGES |
    	...
    	if (mPackageInfo != null && mApplicationInfo != null) {
    		if ((mApplicationInfo.flags & ApplicationInfo.FLAG_SYSTEM) != 0) {
    	        isSystem = true;
    			return !isSystemPackage(manager, mPackageInfo) && !mApplicationInfo.enabled;
			}
		}
    	return true;
    }
    ...
    public void onDragStart(DragSource source, Object info, int dragAction) {
    	if (mLauncher.getWorkspace().getState() != Workspace.State.SPRING_LOADED) {
			mLauncher.fullScreen(true);
		}
        ...
        if (useUninstallLabel) {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN_MR2) {
                ...
                if (restrictions.getBoolean("no_control_apps", false)
    ...
    public void onDragEnd() {
        super.onDragEnd();
        mActive = false;
    	if (!mLauncher.getIsStatusBarVisible()) {
			mLauncher.fullScreen(false);
		}
    }
        ...
        Runnable onAnimationEndRunnable = new Runnable() {
            ...
            public void run() {
                ...
                if (QCConfig.autoDeleteAndAddEmptyScreen) {
					...
					mLauncher.getWorkspace().cellsShowAddOrDeleteButton(true, true);
				}
				/// M.
    ...
    private void completeDrop(DragObject d) {
        ...
        } else if (isWorkspaceOrFolderApplication(d) || isWorkspaceOrFolderShortcutWidget(d)
                || ((d.dragInfo instanceof ShortcutInfo )) && (d.dragSource.supportsDeleteDropTarget())) {
