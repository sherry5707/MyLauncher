...
import com.qingcheng.home.AutoInstallsLayout.LayoutParserCallback;
...
import com.qingcheng.home.config.ProviderConfig;
...
public class LauncherProvider extends ContentProvider {
    ...
    // Add for MyUI Jing.Wu 20161027 start
    private static LauncherApplication mApplication;
    private static void setDefBrowserName(String name) {
    	if (mApplication!=null) {
    		SharedPreferences mPrefs = mApplication.getSharedPreferences(QCPreference.PREFERENCE_NAME, Context.MODE_PRIVATE);
        	SharedPreferences.Editor mEditor = mPrefs.edit();
        	mEditor.putString(QCPreference.KEY_DEFAULT_BROWSER, name);
        	mEditor.commit();
		}
    }
    public static String getDefaultBrowserName() {
    	if (mApplication==null) {
			...
			SharedPreferences mPrefs = mApplication.getSharedPreferences(QCPreference.PREFERENCE_NAME, Context.MODE_PRIVATE);
	    	String mBrowserName = mPrefs.getString(QCPreference.KEY_DEFAULT_BROWSER, null);
	    	return mBrowserName;
		}
    }
    // Add for MyUI Jing.Wu 20161027 end
    ...
    public boolean onCreate() {
        ...
        sIsTablet = !mApplication.getIsPhone();
        ...
        if (sp.getBoolean(EMPTY_DATABASE_CREATED, false)) {
            ...
            if (loader == null) {
                loader = new SimpleWorkspaceLoader(mOpenHelper, getContext().getResources(),
                        getDefaultWorkspaceResourceId(getContext()));
            }
    ...
    private static int getDefaultWorkspaceResourceId(Context context) {
    	if (LauncherApplication.getHasDeviceLayoutFile() && LauncherApplication.getHasDeviceLayoutParameter()) {
			int i = context.getResources().getIdentifier(LauncherApplication.getDeviceName(), "xml", context.getPackageName());
			if (i!=0) {
				return i;
			}
		}
    ...
    private static class DatabaseHelper extends SQLiteOpenHelper implements LayoutParserCallback {
        ...
        // Add for MyUI Jing.Wu 20161027 start
        private static final String ATTR_DEFAULT_BROWSER = "defaultBrowser";
        // Add for MyUI Jing.Wu 20161027 end
        ...
        DatabaseHelper(Context context) {
            ...
            /// M: [OP09]Add for edit and hide apps.
        ...
        public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
            ...
            if (version < 20) {
                ...
                try {
					...
					if (systemProperties!=null) {
						...
						operator = (String)get.invoke(systemProperties, "ro.operator.optr", null);
					}
				} catch (Exception e) {
					// TODO: handle exception
					e.printStackTrace();
				}
                Log.d(TAG, "onUpgrade, operator = " + operator);
            ...
            // add by xiaolin.he 20151201 start . for duplicate BaiduSearch icon in Launcher
            if (version < 21) {
                updateBaiduSearchComponentName(db);
                version = 21;
            }
            // add by xiaolin.he 20151201 end .

            if(version < 22){
                Launcher.addDumpLog(TAG, "onUpgrade: version = " + version, true);
                version = 22;
//                new Thread(new Runnable() {
//                    @Override
//                    public void run() {
//                        final String targetDir = mContext.getFilesDir().getPath() + QCPreference.THEME_DIR;
//                        try {
//                            UnzipUtility.unzip(QCPreference.getInternalThemePath() + QCPreference.THEME_ZIP, targetDir);
//                            SharedPreferences pref = mContext.getSharedPreferences(QCPreference.PREFERENCE_NAME, Context.MODE_PRIVATE);
//                            SharedPreferences.Editor editor = pref.edit();
//                            editor.putBoolean(QCPreference.KEY_CUSTOM_THEME, true);
//                            editor.apply();
//                        } catch (IOException e) {
//                            e.printStackTrace();
//                            Log.e(TAG, "run: upgrade " + e.getMessage(), e);
//                        }
//                    }
//                }).start();
            }

            Log.e(TAG, "onUpgrade: version = " + version);
            Launcher.addDumpLog(TAG, "onUpgrade: version = " + version, true);
            if(version < 23){
                version = 23;
                updatePeopleShortcuts(db);
            }

            if(version < 24){//change music app
                version = 24;
                updateMusicShortcuts(db);
            }
        ...
        // add by xiaolin.he 20151201 start . for duplicate BaiduSearch icon in Launcher
        private void updateBaiduSearchComponentName(SQLiteDatabase db) {
            String intentOld = "#Intent;action=android.intent.action.MAIN;category=android.intent.category.LAUNCHER;launchFlags=0x10200000;component=com.baidu.searchbox/.MainActivity;l.profile=0;end";
            String intentNew = "#Intent;action=android.intent.action.MAIN;category=android.intent.category.LAUNCHER;launchFlags=0x10200000;component=com.baidu.searchbox/.SplashActivity;l.profile=0;end";
            db.delete(TAG_FAVORITES, "intent like '%com.baidu.searchbox/.SplashActivity%'", null);
            String sql21 = "UPDATE favorites SET intent='" + intentNew + "' WHERE intent like '%com.baidu.searchbox/.MainActivity%'";
            db.execSQL(sql21);
        }
        ...
        private boolean updateMusicShortcuts(SQLiteDatabase db) {
            final String selectWhere = buildOrWhereString(Favorites.ITEM_TYPE,
                    new int[] { Favorites.ITEM_TYPE_APPLICATION });

            Log.d(TAG, "updateMusicShortcuts: start");
            Launcher.addDumpLog(TAG, "updateMusicShortcuts start", true);
            Cursor c = null;
            //component=com.android.contacts/.activities.PeopleActivity
            final String targetPackage = "com.android.music";
            final String targetClassName = "com.android.music.MusicBrowserActivity";
            db.beginTransaction();
            try {
                ...
                Log.d(TAG, "updateMusicShortcuts found upgrade cursor count=" + c.getCount());
                ...
                while (c.moveToNext()) {
                    ...
                    if (intentUri != null) {
                        try {
                            ...
                            final String orgPackage = componentName.getPackageName();
                            final String orgClassName = componentName.getClassName();

                            if (targetPackage.equals(orgPackage) && targetClassName.equals(orgClassName)) {
                                final ContentValues values = new ContentValues();
                                values.put(LauncherSettings.Favorites.INTENT,
                                        "#Intent;action=android.intent.action.MAIN;category=android.intent.category.LAUNCHER;launchFlags=0x10200000;component=com.ragentek.vogamusic/com.ting.main.WelcomeActivity;end");
                                String updateWhere = Favorites._ID + "=" + favoriteId;
                                db.update(TABLE_FAVORITES, values, updateWhere, null);
                            }
                        } catch (RuntimeException ex) {
                            Log.e(TAG, "updateMusicShortcuts problem upgrading shortcut", ex);
            ...
            Log.d(TAG, "updateMusicShortcuts: end");
            Launcher.addDumpLog(TAG, "updateMusicShortcuts end", true);
            return true;
        }

        private boolean updatePeopleShortcuts(SQLiteDatabase db) {
            final String selectWhere = buildOrWhereString(Favorites.ITEM_TYPE,
                    new int[] { Favorites.ITEM_TYPE_APPLICATION });

            Log.e(TAG, "updatePeopleShortcuts: start");
            Launcher.addDumpLog(TAG, "updatePeopleShortcuts start", true);
            Cursor c = null;
            //component=com.android.contacts/.activities.PeopleActivity
            final String targetPackage = "com.android.contacts";
            final String targetClassName = "com.android.contacts.activities.PeopleActivity";
            db.beginTransaction();
            try {
                // Select and iterate through each matching widget
                c = db.query(TABLE_FAVORITES,
                ...
                Log.e(TAG, "updatePeopleShortcuts found upgrade cursor count=" + c.getCount());
                ...
                while (c.moveToNext()) {
                    ...
                    if (intentUri != null) {
                        try {
                            ...
                            Log.e(TAG, "updatePeopleShortcuts updatePeopleShortcuts =" + intent.toString());
                            final ComponentName componentName = intent.getComponent();
                            final String orgPackage = componentName.getPackageName();
                            final String orgClassName = componentName.getClassName();
                            ...
                            if (targetPackage.equals(orgPackage) && targetClassName.equals(orgClassName)) {
                                    ...
                                    final ContentValues values = new ContentValues();
                                    values.put(LauncherSettings.Favorites.INTENT,
                                            "#Intent;action=android.intent.action.MAIN;category=android.intent.category.LAUNCHER;launchFlags=0x10200000;component=com.android.dialer/com.android.contacts.PeopleActivity;end");

                                    String updateWhere = Favorites._ID + "=" + favoriteId;
                                    Log.e(TAG, "updatePeopleShortcuts: update");
                                Launcher.addDumpLog(TAG, "updatePeopleShortcuts update", true);
                                    db.update(TABLE_FAVORITES, values, updateWhere, null);
                            }
                        } catch (RuntimeException ex) {
                            Log.e(TAG, "updatePeopleShortcuts problem upgrading shortcut", ex);
                        } catch (URISyntaxException e) {
                            e.printStackTrace();
                        }
                    }
                ...
                Log.w(TAG, "updatePeopleShortcuts problem while upgrading contacts", ex);
                Launcher.addDumpLog(TAG, "updatePeopleShortcuts " + ex.getLocalizedMessage(), true);
                return false;
            } finally {
                db.endTransaction();
                if (c != null) {
            ...
            Log.e(TAG, "updatePeopleShortcuts: end");
            Launcher.addDumpLog(TAG, "updatePeopleShortcuts end", true);
            return true;
        }
        ...
        private long addAppShortcutByUri(SQLiteDatabase db, ContentValues values,
                String intentUri, XmlResourceParser parser) {
            Intent metaIntent;
            try {
            ...
            // Add for MyUI Jing.Wu 20161027 start
        	if (getDefaultBrowserName() == null) {
        		String strContainer = getAttributeValue(parser, ATTR_CONTAINER);
            	if (strContainer!=null && Long.valueOf(strContainer)==LauncherSettings.Favorites.CONTAINER_HOTSEAT) {
            		final String defaultBrowser = getAttributeValue(parser, ATTR_DEFAULT_BROWSER);
                    if (!TextUtils.isEmpty(defaultBrowser) && defaultBrowser.equals("true")) {
                    	ComponentName mComponentName = intent.getComponent();
    					setDefBrowserName(mComponentName.getClassName());
    				}
				}
			}
            // Add for MyUI Jing.Wu 20161027 end
        ...
        private long addAppShortcut(SQLiteDatabase db, ContentValues values,
    		 ...
    		 */
        	 if(QCLog.DEBUG){
             	QCLog.i("ItemInfo", "=addAppShortcut= className:" +className +" class:"+ this.getClass()  ,true);
             }
            if (!TextUtils.isEmpty(packageName) && !TextUtils.isEmpty(className)) {
                ...
                try {
                    ...
                    try {
                    	if (QCConfig.supportAddEmptyIcons) {
							String[] emptyAppTitle = mContext.getResources().getStringArray(R.array.empty_app_title);
							String[] emptyAppPackageNames = mContext.getResources().getStringArray(R.array.empty_app_packagename);
							String[] emptyAppClassNames = mContext.getResources().getStringArray(R.array.empty_app_classname);
							if (emptyAppTitle!=null && emptyAppTitle.length>0 && 
									emptyAppPackageNames!=null && emptyAppPackageNames.length>0 && 
									emptyAppClassNames!=null && emptyAppClassNames.length>0 && 
									emptyAppTitle.length==emptyAppClassNames.length && 
									emptyAppPackageNames.length==emptyAppClassNames.length) {
								for (int i = 0; i < emptyAppClassNames.length; i++) {
									if (className.equals(emptyAppClassNames[i])) {
										title = emptyAppTitle[i];
										cn = new ComponentName(emptyAppPackageNames[i], emptyAppClassNames[i]);
										break;
									}
								}
							}
                    ...
                    return addAppShortcut(db, values, title, intent);
							
                    //sunfeng modify @20151020  show Icon Null :
                } catch ( Exception e) {
                ...
                return addAppShortcutByUri(db, values, uri, parser);
            } else {
        ...
        private long addAppShortcut(SQLiteDatabase db, ContentValues values, String title,
    		 ...
    		 */
        	 if(QCLog.DEBUG){
             	QCLog.i("ItemInfo", " addAppShortcut= title:" +title +" class:"+ this.getClass()  ,true);
             }
        ...
        private long addUriShortcut(SQLiteDatabase db, ContentValues values, Resources res,
    		 ...
    		 */
        	 if(QCLog.DEBUG){
             	QCLog.i("ItemInfo", " addUriShortcut= title:" +res.getString(titleResId) +" class:"+ this.getClass() ,true);
             }
    ...
    private static String getAttributeValue(XmlResourceParser parser, String attribute) {
        String value = parser.getAttributeValue(
                "http://schemas.android.com/apk/res-auto/com.qingcheng.home", attribute);
        if (value == null) {
    ...
    /// M: [OP09]Add for edit and hide apps.Start @{
