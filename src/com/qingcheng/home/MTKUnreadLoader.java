...
import com.qingcheng.home.util.XmlUtils;
...
public class MTKUnreadLoader extends BroadcastReceiver {
    ...
    private static int workSpaceTopMarg = -1;
    private static int workSpaceRightMarg = -1;
    private static int hotseatTopMarg = -1;
    private static int hotseatRightMarg = -1;
    private static int folderTopMarg = -1;
    private static int folderRightMarg = -1;
    ...
    public void onReceive(final Context context, final Intent intent) {
        ...
        if ("com.mediatek.action.UNREAD_CHANGED".equals(action)) {
            final ComponentName componentName = (ComponentName) intent
                    .getParcelableExtra("com.mediatek.intent.extra.UNREAD_COMPONENT");
    ...
    public void initialize(UnreadCallbacks callbacks) {
        ...
        if (LauncherAppState.getInstance().getDynamicGrid().getDeviceProfile().numRows <= 4) {
        	workSpaceTopMarg = mApplication.getPxforYLayout(true, R.dimen.workspace_unread_margin_top_4x4, 0);
		} else {
			workSpaceTopMarg = mApplication.getPxforYLayout(true, R.dimen.workspace_unread_margin_top_5x4, 0);
		}
        workSpaceRightMarg = mApplication.getPxforYLayout(true, R.dimen.workspace_unread_margin_right, 0);
        hotseatTopMarg = mApplication.getPxforYLayout(true, R.dimen.hotseat_unread_margin_top, 0);
        hotseatRightMarg = mApplication.getPxforYLayout(true, R.dimen.hotseat_unread_margin_right, 0);
        folderTopMarg = mApplication.getPxforYLayout(true, R.dimen.folder_unread_margin_top, 0);
        folderRightMarg = mApplication.getPxforYLayout(true, R.dimen.folder_unread_margin_right, 0);
    }
    ...
    static void drawUnreadEventIfNeed(Canvas canvas, View icon) {
        ...
        if (info != null && info.unreadNum > 0) {
            ...
            Drawable drawable = res.getDrawable(R.drawable.unread_background);
            int unreadBgWidth = drawable.getIntrinsicWidth();
            int unreadBgHeight = drawable.getIntrinsicHeight();
            ...
            drawable.setBounds(unreadBgBounds);

            int unreadMarginTop = 0;
            int unreadMarginRight = 0;
            if (info instanceof ShortcutInfo) {
                if (info.container == (long) LauncherSettings.Favorites.CONTAINER_HOTSEAT) {
                    unreadMarginTop = hotseatTopMarg;//(int) res.getDimension(R.dimen.hotseat_unread_margin_top);
                    unreadMarginRight = hotseatRightMarg;//(int) res.getDimension(R.dimen.hotseat_unread_margin_right);
                } else if (info.container == (long) LauncherSettings.Favorites.CONTAINER_DESKTOP) {
                    unreadMarginTop = workSpaceTopMarg;//(int) res.getDimension(R.dimen.workspace_unread_margin_top);
                    unreadMarginRight = workSpaceRightMarg;//(int) res.getDimension(R.dimen.workspace_unread_margin_right);
                } else {
                    unreadMarginTop = folderTopMarg;//(int) res.getDimension(R.dimen.folder_unread_margin_top);
                    unreadMarginRight = folderRightMarg;//(int) res.getDimension(R.dimen.folder_unread_margin_right);
                }
