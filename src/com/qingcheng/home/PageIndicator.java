...
public class PageIndicator extends LinearLayout {
    ...
    public static class PageMarkerResources {
        ...
        public PageMarkerResources() {
            activeId = R.drawable.workspace_page_view_current_color;
            inactiveId = R.drawable.workspace_page_view_default_color;
        }
    ...
    void offsetWindowCenterTo(int activeIndex, boolean allowAnimations) {
        ...
        for (int i = 0; i < mMarkers.size(); ++i) {
            ...
            if(isNewsPage && i == 0 ){
                marker.setMarkerDrawables(R.drawable.qingcheng_page_news_page, R.drawable.qingcheng_page_news_page);
            }
    ...
    void addMarker(int index, PageMarkerResources marker, boolean allowAnimations) {
    	if (QCLog.DEBUG) {
			QCLog.d(TAG, "addMarker() and index = "+index);
		}
	...
	// Add for AddExtraEmptyScreen Jing.Wu 20151218 start
    void changeMarkerVisibility(int index, boolean visible) {
    	if (QCLog.DEBUG) {
			QCLog.d(TAG, "changeMarkerVisibility() and index = "+index+", visible= "+visible);
		}
    	
    	if (index>=0 && mMarkers!=null && !mMarkers.isEmpty() && mMarkers.size()>index) {
			PageIndicatorMarker marker = mMarkers.get(index);
			if (marker!=null) {
				marker.setVisibility(visible?View.VISIBLE:View.GONE);
				//offsetWindowCenterTo(mActiveMarkerIndex, true);
			}
		}
    }
	// Add for AddExtraEmptyScreen Jing.Wu 20151218 end

    void removeMarker(int index, boolean allowAnimations) {
    	if (QCLog.DEBUG) {
			QCLog.d(TAG, "removeMarker() and index = "+index);
		}
