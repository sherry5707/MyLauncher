...
public abstract class PagedView extends ViewGroup implements ViewGroup.OnHierarchyChangeListener {
    ...
    protected static int INVALID_PAGE = -1;
    ...
    // Add for MyUI---20150721
    protected int mScrollDirection = 0;
    // Add for MyUI---20150723
    protected Rect mChildrenCellRect = new Rect(-1, -1, -1, -1);

	// Add for MyUI when workspace contain custompage and extraemptypage at the same time Jing.Wu 20151224 start
    protected int mShouldStartPageNumb;
    protected int mShouldEndPageNumb = 1;
    protected int mShouldStartScrollX;
    protected int mShouldEndScrollX;
	// Add for MyUI when workspace contain custompage and extraemptypage at the same time Jing.Wu 20151224 end
    
    // Add for workspace overview effect Jing.Wu 20151230 start
    protected float mReorderingChildScale = 1.15f;
    ...
    // Add for MyUI---20150814
    private boolean isUseMinScaleWhenTouchDown = false;
	// Add for AddExtraEmptyScreen Jing.Wu 20151227 start
    protected boolean needSnapToExtraEmptyScreen = false;
	// Add for AddExtraEmptyScreen Jing.Wu 20151227 end
	...
	// Modify for AddExtraEmptyScreen Jing.Wu 20151227 start
    protected boolean mReorderingStarted = false;
	// Modify for AddExtraEmptyScreen Jing.Wu 20151227 end
    ...
    public boolean mInOverviewMode = false;
...
//        mCurrentPage = 0;
    ...
    //sunfeng modify @20151021 for JL610 JLLYL-1170 end:
    ...
    protected void onAttachedToWindow() {
        ...
        if (QCLog.DEBUG) {
			QCLog.d(TAG, "onAttachedToWindow() and mPageIndicator == null? "+(mPageIndicator == null)
					+", mPageIndicatorViewId > -1? "+(mPageIndicatorViewId > -1));
		}
    ...
    protected void onDetachedFromWindow() {
        ...
        mPageIndicator = null;
        if (QCLog.DEBUG) {
			QCLog.d(TAG, "onDetachedFromWindow()");
		}
    }
    ...
    int getNextPage() {
    	if (QCLog.DEBUG) {
			QCLog.d(TAG, "getNextPage() and mNextPage = "+mNextPage);
		}
    ...
    protected void updateCurrentPageScroll() {
        ...
        int newX = 0;
//        Log.e(TAG,"updateCurrentPageScroll: mCurrentPage = " + mCurrentPage + " getPageCount = " + getPageCount());
        if (0 <= mCurrentPage && mCurrentPage < getPageCount()) {
            newX = getScrollForPage(mCurrentPage);
        }
//        Log.e(TAG,"updateCurrentPageScroll: newx = " + newX);
        scrollTo(newX, 0);
        mScroller.setFinalX(newX);
        forceFinishScroller();
    }

    void setCurrentPageReal(int page) {
//        try {
//            if ((page == 0 && mCurrentPage == 1) || page == -1) {
//                throw new IllegalArgumentException("setCurrentPageTest");
//            }
//        } catch (Exception e) {
//            Log.e(TAG, "set currentPage test page = " + page);
//            e.printStackTrace();
//        }
        mCurrentPage = page;
    }
...
//        mCurrentPage = getNextPage();
        setCurrentPageReal(getNextPage());
...
//        Log.e(TAG, "validateNewPage: start validatedPage = " + validatedPage);
...
//        Log.e(TAG, "validateNewPage: mShouldStartPageNumb   = " + mShouldStartPageNumb);
//        Log.e(TAG, "validateNewPage: mShouldEndPageNumb   = " + mShouldEndPageNumb);
//        Log.e(TAG, "validateNewPage: validatedPage   = " + validatedPage);
        // Ensure that it is clamped by the actual set of children in all cases
    	// Modify for MyUI when workspace contain custompage and extraemptypage at the same time Jing.Wu 20151224 start
        validatedPage = Math.max(mShouldStartPageNumb, Math.min(validatedPage, mShouldEndPageNumb));
//    	 validatedPage = Math.max(0, Math.min(validatedPage, getPageCount() - 1));
        // Modify for MyUI when workspace contain custompage and extraemptypage at the same time Jing.Wu 20151224 end
//        Log.e(TAG, "validateNewPage: end validatedPage   = " + validatedPage);
    ...
    void setCurrentPage(int currentPage) {
    	...
    	// Modify for AddExtraEmptyScreen Jing.Wu 20151227 start
        if (!QCConfig.autoDeleteAndAddEmptyScreen && needSnapToExtraEmptyScreen) {
			snapToPage(mCurrentPage+1, 0);
			needSnapToExtraEmptyScreen = false;
		} else {
            mForceScreenScrolled = true;
//          mCurrentPage = validateNewPage(currentPage);
            setCurrentPageReal(validateNewPage(currentPage));
            updateCurrentPageScroll();
            notifyPageSwitchListener();
            invalidate();
		}
//        if (QCLog.DEBUG) {
//			QCLog.d(TAG, "setCurrentPage() and mCurrentPage = "+mCurrentPage);
//		}
    	// Modify for AddExtraEmptyScreen Jing.Wu 20151227 end
    }
    ...
    protected void pageBeginMoving() {
        if (!mIsPageMoving) {
            Trace.beginSection("pageBeginMoving");
            mIsPageMoving = true;
            onPageBeginMoving();
            Trace.endSection();
        }
    ...
    protected void pageEndMoving() {
        if (mIsPageMoving) {
            Trace.beginSection("pageEndMoving");
            mIsPageMoving = false;
            onPageEndMoving();
            Trace.endSection();
        }
    ...
    public void scrollTo(int x, int y) {
        ...
        boolean isXBeforeFirstPage = isRtl ? (x > mShouldEndScrollX) : (x < mShouldStartScrollX);
        boolean isXAfterLastPage = isRtl ? (x < mShouldStartScrollX) : (x > mShouldEndScrollX);
        if (LauncherLog.DEBUG_DRAW) {
            LauncherLog.d(TAG, "scrollTo: x = " + x + ", y = " + y + ", mOverScrollX = "
                    ...
                    + isXAfterLastPage + ", mAllowOverScroll = " + mAllowOverScroll + ", this = " + this);
        }
        if (isXBeforeFirstPage) {
            /// M: modify to cycle sliding screen.
            if (isSupportCycleSlidingScreen()) {
                mOverScrollX = mShouldStartScrollX;
                super.scrollTo(x, y);
            } else {
                super.scrollTo(mShouldStartScrollX, y);
                if (mAllowOverScroll) {
                    if (isRtl) {
                        overScroll(x - mShouldEndScrollX);
                    } else {
                        overScroll(x - mShouldStartScrollX);
                    }
                }
            }
            ...
            /// M: modify to cycle sliding screen.
            if (isSupportCycleSlidingScreen()) {
                mOverScrollX = mShouldEndScrollX;
                super.scrollTo(x, y);
            } else {
                super.scrollTo(mShouldEndScrollX, y);
                if (mAllowOverScroll) {
                    if (isRtl) {
                        overScroll(x - mShouldStartScrollX);
                    } else {
    ...
    private void sendScrollAccessibilityEvent() {
        ...
        if (am.isEnabled()) {
        	...
        	if (QCLog.DEBUG) {
				QCLog.d(TAG, "sendScrollAccessibilityEvent() and action == AccessibilityNodeInfo.ACTION_SCROLL_FORWARD ? "+
						(ev.getAction() == AccessibilityNodeInfo.ACTION_SCROLL_FORWARD));
			}
        }
    }
    ...
    protected boolean computeScrollHelper() {
        if (mScroller.computeScrollOffset()) {
        	...
        	if (QCLog.DEBUG) {
				QCLog.d(TAG, "computeScrollHelper() and mNextPage != INVALID_PAGE");
			}

            sendScrollAccessibilityEvent();

//            mCurrentPage = validateNewPage(mNextPage);
            setCurrentPageReal(validateNewPage(mNextPage));
            mNextPage = INVALID_PAGE;
            notifyPageSwitchListener();
    ...
    public void computeScroll() {
    	if (QCLog.DEBUG) {
			QCLog.d(TAG, "pagedView start computeScroll()");
		}
        computeScrollHelper();
    }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
    	if (QCLog.DEBUG) {
			QCLog.d(TAG, "pagedView order onMeasure() and (!mIsDataReady || getChildCount() == 0) ? "+(!mIsDataReady || getChildCount() == 0));
		}
        ...
        if (DEBUG) Log.d(TAG, "PagedView.mInsets: " + mInsets.left + ", " + mInsets.top+ ", " + mInsets.right+ ", " + mInsets.bottom);
        if (DEBUG) Log.d(TAG, "PagedView.parentWidthSize: " + parentWidthSize);
        if (DEBUG) Log.d(TAG, "PagedView.mMinScale: " + mMinScale);
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            if (child.getVisibility() != GONE) {
                ...
                if (!lp.isFullScreenPage) {
                    ...
                    if (lp.isFullScreenPage) {
                        if(grid.isLandscape){
                            childWidth = getViewportWidth() - LauncherApplication.navBarHeight;
                        }else{
                            childHeight = getViewportHeight() - LauncherApplication.navBarHeight;
                        }
                    }
                ...
                if (QCLog.DEBUG) {
					QCLog.d(TAG, "measure-child "+i+": left = "+child.getLeft()+", translationX = "+child.getTranslationX()
							+", X = "+child.getX()
							+", childWidth = "+childWidth
							+", childWidthMeasureSpec = "+childWidthMeasureSpec);
				}
            }
        }
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
    	if (QCLog.DEBUG) {
			QCLog.d(TAG, "test order onLayout() and mReorderingStarted = " + mReorderingStarted
					+ ", test isReordering = " + isReordering(true)
					+ ", mIsReordering = " + mIsReordering);
		}
        if (!mIsDataReady || getChildCount() == 0) {
            return;
        }
        ...
        if (DEBUG) Log.d(TAG, "PagedView.onLayout() and changed = "+changed+", left = "+left+", right = "+right);
        final int childCount = getChildCount();
        ...
        for (int i = startIndex; i != endIndex; i += delta) {
            ...
            if (child.getVisibility() != View.GONE) {
                ...
                final int childWidth = grid.isLandscape ? (getViewportWidth() - LauncherApplication.navBarHeight ): child.getMeasuredWidth();
                final int childHeight = child.getMeasuredHeight();

                if (DEBUG) Log.d(TAG, "\tlayout-child" + i + ": " + childLeft + ", " + childTop);
                if (QCLog.DEBUG) {
					QCLog.d(TAG, "\tlayout-child" + i + ", offsetX = "+offsetX+", getPaddingLeft = "+child.getPaddingLeft()+
							", translationX = "+child.getTranslationX()+", mScrollX = "+child.getScrollX());
				}
        ...
        if (childCount > 0) {
            ...
            if(isSupportCycleSlidingScreen()){
                //mScrollRightX = mMaxScrollX + getMeasuredWidth();
                //mScrollLeftX = -getMeasuredWidth();
                mScrollRightX = mMaxScrollX + (int)LauncherApplication.getScreenWidthPixel();
                mScrollLeftX = getScrollForPage(0)-(int)LauncherApplication.getScreenWidthPixel();
            }else{
                mScrollRightX = mMaxScrollX;
                mScrollLeftX = getScrollForPage(0);
            }
        } else {
            mMaxScrollX = 0;
        }
...
//        LauncherLog.d(TAG, "pagedView onLayout: mShouldStartPageNumb = " + mShouldStartPageNumb + ", mShouldEndPageNumb = " + mShouldEndPageNumb
//                + ", mShouldStartScrollX = " + mShouldStartScrollX + ", mShouldEndScrollX = " + mShouldEndScrollX);
    	// Add for MyUI when workspace contain custompage and extraemptypage at the same time Jing.Wu 20151224 end

        if (QCLog.DEBUG) {
			QCLog.d(TAG, "PagedView.onLayout() and mScroller.isFinished() ? "+mScroller.isFinished());
		}
...
//                Log.d(TAG, "onLayout: pagedView current page = " + getCurrentPage());
            }
        }
    ...
    public void onChildViewAdded(View parent, View child) {
    	if (QCLog.DEBUG) {
			QCLog.d(TAG, "onChildViewAdded() and (mPageIndicator!=null)? "+(mPageIndicator != null)
					+", !isReordering(false)? "+(!isReordering(false)));
		}
    ...
    public void onChildViewRemoved(View parent, View child) {
    	if (QCLog.DEBUG) {
			QCLog.d(TAG, "onChildViewRemoved() and parent = "+parent+", child = "+child);
		}
        mForceScreenScrolled = true;
        updateFreescrollBounds();
        invalidate();
    ...
    protected void getVisiblePages(int[] range) {
        ...
        if (pageCount > 0) {
            /// M: modify to cycle sliding screen.
        	// Modify for MyUI when workspace contain custompage and extraemptypage at the same time Jing.Wu 20151224 start
            if (isSupportCycleSlidingScreen() && (getScrollX() < mShouldStartScrollX || getScrollX() > mShouldEndScrollX)) {
                range[0] = mShouldEndPageNumb;
                range[1] = mShouldStartPageNumb;
            } else {
                int viewportWidth = getViewportWidth();
                int curScreen = 0;

                int count = getChildCount();
                int lastChildPageNumb = Math.min(mShouldEndPageNumb, (mReorderingStarted&&!QCConfig.autoDeleteAndAddEmptyScreen?count-2:count-1));
                for (int i = mShouldStartPageNumb; i <= lastChildPageNumb; i++) {
                	// Modify for MyUI when workspace contain custompage and extraemptypage at the same time Jing.Wu 20151224 end
                ...
                if (isSupportCycleSlidingScreen() && rightScreen < leftScreen) {
                    ...
                    for (int i = pageCount - 1; i >= 0; i--) {
                        ...
                        if (mForceDrawAllChildrenNextFrame ||
                            ...
                            if(v.getAlpha() != QCConfig.defaultAlpha && mIsPageMoving == false){
    ...
    protected Bitmap mWallpaperBitmap;
    ...
    public void addFocusables(ArrayList<View> views, int direction, int focusableMode) {
        ...
        if (direction == View.FOCUS_LEFT) {
            if (mCurrentPage >=1) {
                if(getPageAt(mCurrentPage - 1)!=null) {
    ...
    private boolean isTouchPointInViewportWithBuffer(int x, int y) {
    	...
    	if (QCLog.DEBUG) {
			QCLog.d(TAG, "isTouchPointInViewportWithBuffer() and mTmpRect.left = "+mTmpRect.left
					+", mTmpRect.top = "+mTmpRect.top
					+", mTmpRect.right = "+mTmpRect.right
					+", mTmpRect.bottom = "+mTmpRect.bottom
					+", x = "+x+", y = "+y);
		}
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        switch (action & MotionEvent.ACTION_MASK) {
            ...
            case MotionEvent.ACTION_DOWN: {
                ...
                if (LauncherLog.DEBUG) {
                    LauncherLog.d(TAG, "onInterceptTouchEvent touch down: mScroller.isFinished() = "
                    		+(mScroller.isFinished())+", (xDist < mTouchSlop / 3)? "+(xDist < mTouchSlop / 3)
                    		+", finishedScrolling = " + finishedScrolling 
                    		+ ", mScrollX = " + getScrollX() + ", xDist = " + xDist
                ...
                if (LauncherLog.DEBUG) {
                    LauncherLog.d(TAG, "onInterceptTouchEvent touch down: mScroller.isFinished() = "
                    		+(mScroller.isFinished())+"finishedScrolling = "
                            + finishedScrolling + ", mScrollX = " + getScrollX() + ", xDist = " + xDist
                            + ", mTouchState = " + mTouchState + ", this = " + this);
                }
                
                // Add for MyUI---20150814
                isUseMinScaleWhenTouchDown = mUseMinScale;
    ...
    protected void dampedOverScroll(float amount) {
    	...
    	// Modify for MyUI when workspace contain custompage and extraemptypage at the same time Jing.Wu 20151224 start
        if (amount < 0) {
            mOverScrollX = mShouldStartScrollX + overScrollAmount;
            super.scrollTo(mOverScrollX, getScrollY());
        } else {
            mOverScrollX = mShouldEndScrollX + overScrollAmount;
            super.scrollTo(mOverScrollX, getScrollY());
        }
    	// Modify for MyUI when workspace contain custompage and extraemptypage at the same time Jing.Wu 20151224 end
        if (QCLog.DEBUG) {
			QCLog.d(TAG, "dampedOverScroll() and scrollto "+mOverScrollX);
		}
        invalidate();
    }
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        case MotionEvent.ACTION_DOWN:
            ...
            if (!mScroller.isFinished()) {
            	if (QCLog.DEBUG) {
					QCLog.d(TAG, "onTouchEvent() and touch down, !mScroller.isFinished() is true");
				}
        ...
        case MotionEvent.ACTION_UP:
            if (mTouchState == TOUCH_STATE_SCROLLING) {
                ...
                if (!mFreeScroll) {
                	...
                	// Modify for MyUI when workspace contain custompage and extraemptypage at the same time Jing.Wu 20151224 start
                    if ((!isSupportCycleSlidingScreen() && (getScrollX()<getScrollForPage(mShouldStartPageNumb)||getScrollX()>getScrollForPage(mShouldEndPageNumb)))
                        	// Modify for MyUI when workspace contain custompage and extraemptypage at the same time Jing.Wu 20151224 end
                    		|| (!isUseMinScaleWhenTouchDown && isUseMinScaleWhenTouchDown!=mUseMinScale)
    ...
    int getPageNearestToCenterOfScreen() {
        ...
        final int childCount = getChildCount();
        if (QCLog.DEBUG) {
			QCLog.d(TAG, "getPageNearestToCenterOfScreen() and getViewportOffsetX() = "+getViewportOffsetX()
					+", getScrollX() = "+getScrollX()
					+", getViewportWidth() = "+getViewportWidth()
					+", screenCenter = "+screenCenter);
		}
    	// Modify for MyUI when workspace contain custompage and extraemptypage at the same time Jing.Wu 20151224 start
        for (int i = mShouldStartPageNumb; i < mShouldEndPageNumb+1; ++i) {
        	// Modify for MyUI when workspace contain custompage and extraemptypage at the same time Jing.Wu 20151224 end
            View layout = (View) getPageAt(i);
            // Add for MyUI and fix MyUI stopped problem when layout is null Jing.Wu 20160202 start
            if (layout == null) {
				if (QCLog.DEBUG) {
					QCLog.e(TAG, "getPageNearestToCenterOfScreen() and layout is null, i = "+i);
				}
				continue;
			}
            // Add for MyUI and fix MyUI stopped problem when layout is null Jing.Wu 20160202 end
            int childWidth = layout.getMeasuredWidth();
            int halfChildWidth = (childWidth / 2);
            int childCenter = getViewportOffsetX() + getChildOffset(i) + halfChildWidth;
            int distanceFromScreenCenter = Math.abs(childCenter - screenCenter);
            if (QCLog.DEBUG) {
				QCLog.d(TAG, "getPageAt("+i+"), halfChildWidth = "+halfChildWidth
						+", getChildOffset() = "+getChildOffset(i)
						+", childCenter = "+childCenter
						+", distanceFromScreenCenter = "+distanceFromScreenCenter);
			}
    ...
    protected boolean isInOverScroll() {
    	// Modify for MyUI when workspace contain custompage and extraemptypage at the same time Jing.Wu 20151224 start
        return (mOverScrollX > mShouldEndScrollX || mOverScrollX < mShouldStartScrollX);
    	// Modify for MyUI when workspace contain custompage and extraemptypage at the same time Jing.Wu 20151224 end
    }

    protected int getPageSnapDuration() {
        // Add for switch effect animation Jing.Wu 20150915 start
    	if (Launcher.getIsSwitchAnimationing()) {
			return 550;
    ...
    protected void snapToPage(int whichPage, int duration, boolean immediate,
        ...
        if (QCLog.DEBUG) {
            QCLog.d(TAG, "(PagedView)snapToPage whichPage = " + whichPage);
        }
        whichPage = validateNewPage(whichPage);

        if (QCLog.DEBUG) {
            QCLog.d(TAG, "(PagedView)snapToPage after validateNewPage, whichPage = " + whichPage);
        }
        
        int newX = getScrollForPage(whichPage);
        if (QCLog.DEBUG) {
            QCLog.d(TAG, "snapToPage, newX = "+newX);
        }
        ...
        if (mIsCycleSlidingBound && !mSnapWithTinyVelocity) {
            if (QCLog.DEBUG) {
                QCLog.d(TAG, "mSnapWithTinyVelocity = false");
            }
            
            final int halfScreenSize = getViewportWidth() / 2;
            if (QCLog.DEBUG) {
                QCLog.d(TAG, "halfScreenSize = "+halfScreenSize+", getScrollX() = "+getScrollX()+
    ...
    protected void snapToPage(int whichPage, int delta, int duration, boolean immediate,
        ...
        /// M: ALPS01897331, Widget phase out. And it may be cause memory leak.
        /// M: when snap to page, call the appropriate Widget callback for the current page.
    ...
    public boolean startReordering(View v) {
        ...
        if (mTempVisiblePagesRange[0] <= dragViewIndex &&
            ...
            // Modify for workspace overview effect Jing.Wu 20151230 start
            mDragView.animate().scaleX(mReorderingChildScale).scaleY(mReorderingChildScale).setDuration(100).start();
            // Modify for workspace overview effect Jing.Wu 20151230 end
            mDragViewBaselineLeft = mDragView.getLeft();
    ...
    /**
     * M: Find the Widget View on the given page.
     *
     * @param page
     * @return the Widget view on the given page
     */
    public View getWidgetView(int page) {
        final View whichHostView = getChildAt(page);
        final View mtkWidgetView = searchWidget(whichHostView);
        return mtkWidgetView;
    }

    /**
     * M: Find the Widget View which providerName equals the given providerName.
     *
     * @param hostView
     * @param providerName
     * @return
     */
    public View searchWidget(View hostView, String providerName) {
        if (hostView instanceof ViewGroup) {
            int childCount = ((ViewGroup) hostView).getChildCount();
            for (int i = 0; i < childCount; i++) {
                View mtkWidgetView = searchWidget(((ViewGroup) hostView).getChildAt(i), providerName);
    ...
    /**
     * M: Find the Widget view.
     *
     * @param hostView
     * @return
     */
    private View searchWidget(View hostView) {
        if (hostView instanceof ViewGroup) {
            int childCount = ((ViewGroup) hostView).getChildCount();
            for (int i = 0; i < childCount; i++) {
                View mtkWidgetView = searchWidget(((ViewGroup) hostView).getChildAt(i));
                if (mtkWidgetView != null) {
                    return mtkWidgetView;
                }
            }
    ...
    protected int mScrollRightX;
    protected int mScrollLeftX;
    private boolean isCycleSliding = true;
	public void setCycleSliding(boolean isCycleSliding) {
		this.isCycleSliding = isCycleSliding;
	}
    ...
    protected boolean shouldChildSlide = false;
    protected void drawAnimations(Canvas canvas){
        int halfScreenSize = (mChildrenCellRect.right-mChildrenCellRect.left) / 2;
        // mOverScrollX is equal to getScrollX() when we're within the normal scroll range.
        // Otherwise it is equal to the scaled overscroll position.
        int screenCenter = mOverScrollX + halfScreenSize;
    ...
    protected void drawCircle(Canvas canvas){
    	addGetCircleVisiblePages(mTempVisiblePagesRange);
        int leftScreen = mTempVisiblePagesRange[0];
        int rightScreen = mTempVisiblePagesRange[1];
        int childCount = getChildCount();
        if (QCLog.DEBUG) {
			QCLog.d(TAG, "drawCircle(),leftScreen:"
                    +leftScreen+",+rightScreen:"
                    +rightScreen+",+getChildCount:"
                    +getChildCount());
		}
        if(rightScreen >= childCount || leftScreen < 0){
            View left = null;
            View right = null;
            final long drawingTime = getDrawingTime();
            canvas.save();
            if (QCLog.DEBUG) {
                QCLog.d(TAG, "drawCircle(),getScrollY:"
                        +getScrollY()+",+getScrollX:"
                        +getScrollX()+",+leftScreen:"
                        +leftScreen+",+rightScreen:"
                        +rightScreen+",+getBottom:"
                        +getBottom()+",-getTop:"
                        +getTop()+",-getRight:"
                		+getRight()+",-getLeft:"
                		+getLeft()+", getScreenHeight:"
                		+LauncherApplication.getScreenHeightPixel()+", getScreenWidth:"
                		+LauncherApplication.getScreenWidthPixel());
			}
            canvas.clipRect(getScrollX(), getScrollY(), getScrollX() + getRight() - getLeft(),
                    getScrollY() + getBottom() - getTop());
            
            if(leftScreen < 0){
                if(mIsPageMoving&&getScrollX()<0){
                    drawMasquradeWallpaper(canvas);
                }
                left = getPageAt(childCount-1);
                right = getPageAt(rightScreen);
            }else{
                if(mIsPageMoving&&getScrollX()>mMaxScrollX){
                    drawMasquradeWallpaper(canvas);
                }
                left = getPageAt(leftScreen);
                right = getPageAt(0);
            }
            
            if(mForceDrawAllChildrenNextFrame || shouldDrawChild(left)){
                canvas.save();
                makeSlideEffect(canvas, left, leftScreen);
                drawChild(canvas, left, drawingTime);
                canvas.restore();
            }
            if(mForceDrawAllChildrenNextFrame || shouldDrawChild(right)){
                canvas.save();
                makeSlideEffect(canvas, right, rightScreen);
    ...
    // Fix this for MyUI---20150720
    protected void addGetCircleVisiblePages(int[] range) {
    	final int pageCount = getChildCount();
    	if (pageCount > 0) {
			mScrollDirection = getScrollX() - (int)mDownScrollX;
			if (mScrollDirection<0) {
				...
				// Right
				range[0] = mCurrentPage;
				range[1] = mCurrentPage+1;
			}
		} else {
            range[0] = -1;
            range[1] = -1;
    ...
    protected void getCircleVisiblePages(int[] range){
        final int pageCount = getChildCount();
        ...
        if (pageCount > 0) {
            ...
            if(rightScreen == (pageCount - 1) && rightScreen != 0){
                range[0] = leftScreen;
                range[1] = rightScreen+1;
            }else{
