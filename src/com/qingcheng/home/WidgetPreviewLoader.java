...
public class WidgetPreviewLoader {
    ...
    public Bitmap getPreview(final Object o) {
        ...
        if (QCLog.DEBUG) {
			QCLog.d(TAG, "getPreview() and mPreviewBitmapWidth = "+mPreviewBitmapWidth
    ...
    String SetitngsPm ="com.android.settings.Settings";
    String EmailPm ="com.android.email.activity.Welcome";
    String ContactPm ="com.android.contacts.activities.PeopleActivity";
    String MmsPm ="com.android.mms.ui.BootActivity";
    
    /***
     * 
    ==getPreview== Shortcut:com.android.email/com.android.email2.ui.CreateShortcutActivityEmail  packageName:com.android.email
    ==getPreview== Shortcut:com.android.contacts/com.android.contacts.ContactShortcut  packageName:com.android.contacts
    ==getPreview== Shortcut:com.android.settings/com.android.settings.CreateShortcut  packageName:com.android.settings
    ==getPreview== Widget:AppWidgetProviderInfo(UserHandle{0}/ComponentInfo{com.android.mms/com.android.mms.widget.MmsWidgetProvider})  packageName:com.android.mms
      generateWidgetPreview========getPackageName: com.android.mms toString:AppWidgetProviderInfo(UserHandle{0}/ComponentInfo{com.android.mms/com.android.mms.widget.MmsWidgetProvider})
     * 
     * @param context
     * @param o
     * @param bitmap
     * @return
     */
    private Bitmap getSystemOtherAppIcon(Context context,Object o,Bitmap bitmap){
    	if(context == null){
    		return bitmap;
    	}
    	
    	if(!(o instanceof AppWidgetProviderInfo) && ( ((ResolveInfo)o).activityInfo.name.contains("com.android.email2.ui.CreateShortcutActivityEmail"))){
    		return getThemeOrDefaultIcon(context, EmailPm, bitmap);
    	}
    	if(!(o instanceof AppWidgetProviderInfo) && ( ((ResolveInfo)o).activityInfo.name.contains("com.android.contacts.ContactShortcut"))){
    		return getThemeOrDefaultIcon(context, ContactPm, bitmap);
    	}
    	if(!(o instanceof AppWidgetProviderInfo) && ( ((ResolveInfo)o).activityInfo.name.contains("com.android.settings.CreateShortcut"))){
    		return getThemeOrDefaultIcon(context, SetitngsPm, bitmap);
    	}
    	
    	return bitmap;
    }
    
    private Bitmap getThemeOrDefaultIcon(Context context,String tmp,Bitmap bitmap){
    	
    	Drawable mTmp = mIconCache.getDrawableFromTheme(context, tmp, 0);
    	if(mTmp == null){
    		return bitmap;
    	}
    	BitmapDrawable bmpDrawable = (BitmapDrawable) mTmp;
    ...
    private void writeToDb(Object o, Bitmap preview) {
        ...
        try {
            ...
            LauncherLog.d(TAG, "writeToDb: SQLiteDiskIOException,recreateDb");
            recreateDb();
        } catch (SQLiteCantOpenDatabaseException e) {
            dumpOpenFiles();
            throw e;
        /// M. ALPS01883315, catch the SQLiteReadOnlyDatabaseException
        /// when the cahce has been clear.
        } catch (SQLiteReadOnlyDatabaseException e) {
            LauncherLog.d(TAG, "writeToDb: SQLiteReadOnlyDatabaseException,recreateDb");
            recreateDb();
        /// M.
        }
    }
    ...
    public void clearDb() {
        ...
        /// when the cahce has been clear.
            ...
            LauncherLog.d(TAG, "clearDb: SQLiteReadOnlyDatabaseException,recreateDb");
            recreateDb();
        /// M.
        }
    }
        ...
        new AsyncTask<Void, Void, Void>() {
            public Void doInBackground(Void ... args) {
                ...
                /// when the cahce has been clear.
                    ...
                    LauncherLog.d(TAG, "removeFromDb: SQLiteReadOnlyDatabaseException,recreateDb");
                /// M.
                }
    ...
    private static void removeItemFromDb(final CacheDb cacheDb, final String objectName) {
        new AsyncTask<Void, Void, Void>() {
            public Void doInBackground(Void ... args) {
                SQLiteDatabase db = cacheDb.getWritableDatabase();
                try {
                    LauncherLog.d(TAG, "removeItemFromDb:");
                    db.delete(CacheDb.TABLE_NAME,
                ...
                /// when the cahce has been clear.
                    ...
                    LauncherLog.d(TAG, "removeItemFromDb: recreateDb");
                /// M.
                }
                return null;
            }
    ...
    public Bitmap generateWidgetPreview(AppWidgetProviderInfo info, Bitmap preview) {
        ...
        if (QCLog.DEBUG) {
			QCLog.d(TAG, "generateWidgetPreview() and maxWidth = "+maxWidth
					+", maxHeight = "+maxHeight);
		}
    ...
    public Bitmap generateWidgetPreview(AppWidgetProviderInfo info, int cellHSpan, int cellVSpan,
        ...
        if (widgetPreviewExists) {
            ...
            previewWidth = (int)(previewDrawableWidth * 1.5f);
            previewHeight = (int)(previewDrawableHeight * 1.5f);
            defaultPreview = Bitmap.createBitmap(previewWidth, previewHeight, Config.ARGB_8888);
            if (QCLog.DEBUG) {
    			QCLog.d(TAG, "generateWidgetPreview() and defaultPreview's previewWidth = "+previewWidth
        ...
        if (previewWidth>previewHeight) {
			...
			if (previewHeight > maxPreviewHeight) {
				scale = maxPreviewHeight / (float) previewHeight;
			}
		}
        if (scale != 1f) {
    ...
    private Bitmap generateShortcutPreview(
        ...
        // Render the icon
        Drawable icon = null;
        Bitmap tt = getSystemOtherAppIcon(mContext, info, null);
        
        if(tt == null){
        	icon = mutateOnMainThread(mIconCache.getFullResIcon(info));
        }else{
        	icon = new BitmapDrawable(tt);
        }
        ...
        int scaledIconWidth = Math.min(maxWidth, maxHeight);
        int left = 0;
        int top = 0;
        if (maxWidth>maxHeight) {
			left = (maxWidth-maxHeight)/2;
		} else {
			top = (maxHeight-maxWidth)/2;
		}

        renderDrawableToBitmap(
                icon, tempBitmap, left, top, scaledIconWidth, scaledIconWidth);
        ...
        left = 0;
        top = 0;
        if (maxWidth>mAppIconSize) {
        	left = (maxWidth-mAppIconSize)/2;
		}
        if (maxHeight>mAppIconSize) {
			top = (maxHeight-mAppIconSize)/2;
		}
        renderDrawableToBitmap(icon, preview, left, top, mAppIconSize, mAppIconSize);

        
        
//      return  getSystemOtherAppIcon(mContext, info, preview);
        return preview;
    }
