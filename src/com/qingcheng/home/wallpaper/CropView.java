...
import com.qingcheng.home.wallpaper.photos.views.TiledImageView;
import com.qingcheng.home.wallpaper.photos.views.TiledImageRenderer.TileSource;
...
public class CropView extends TiledImageView implements OnScaleGestureListener {
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        if (action == MotionEvent.ACTION_DOWN) {
            ...
            if (mTouchCallback != null) {
                /// M. alps01914847, the ViewConfiguration.getTapTimeout() is 100ms,
                ///  i think it is too samll to check tap. So, modify it to 200.
                // only do this if it's a small movement
                if (squaredDist < slop &&
                        now < mTouchDownTime + ViewConfiguration.getTapTimeout() * 2) {
        ...
        synchronized (mLock) {
            ...
            if (mRenderer.source != null) {
                ...
                if (edges.top > 0) {
                    ...
                    adjustment[1] = (float) Math.floor(edges.top / scale);
                } else if (edges.bottom < getHeight()) {
                    adjustment[1] = (edges.bottom - getHeight()) / scale;
                }
                for (int dim = 0; dim <= 1; dim++) {
                     /// M. Modify ceil to floor
                    if (coef[dim] > 0) adjustment[dim] = (float) Math.floor(adjustment[dim]);
                }
